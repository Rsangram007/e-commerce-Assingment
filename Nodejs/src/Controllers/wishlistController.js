...
const createCart = async function (req, res) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(productId)) {
      return res
        .status(400)
        .send({ status: false, message: "please provied a valid productId" });
    }
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      return res
        .status(400)
        .send({ status: false, message: "please provied a valid Id" });
    }
    ...
    if (cartExist) {
      ...
      if (!cartId) {
        return res
          ...
          .send({
            status: false,
            message: "cart is already created provide cartId",
          });
      }
      if (!mongoose.Types.ObjectId.isValid(cartId)) {
        return res
          .status(400)
          .send({ status: false, message: "please provied a valid cartId" });
      }
      ...
      // ----------------------------------------------------//
      let productAlreadyAdded = await wishlistModel.findOne({
        userId: userId,
        "items.productId": productId,
      });
      if (productAlreadyAdded) {
        let incQuantity = await wishlistModel
          .findOneAndUpdate(
        ...
        return res
          .status(201)
          .send({ status: true, message: "Success", data: incQuantity });
      }
      let item = { productId: productId, quantity: 1 };
      let productAdd = await wishlistModel
        .findByIdAndUpdate(
          cartId,
          {
        ...
        .populate("items.productId", ["title", "description", "productImage"]);
      return res
        .status(201)
        .send({ status: true, message: "Success", data: productAdd });
    }
    let cartObj = {
      userId: userId,
      items: [{ productId: productId }],
...
const getUsersOfOneproduct = async function (req, res) {
  try {
    ...
    let product = await wishlistModel
