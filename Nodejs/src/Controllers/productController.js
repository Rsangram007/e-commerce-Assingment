...
module.exports = {
  createproduct: async function (req, res) {
    try {
      ...
      if (!title)
        return res
          .status(400)
          .send({ status: false, msg: " Please provide a Title" });
      if (!/^[a-zA-Z0-9.%$#@*&' ]{3,40}$/.test(title))
        return res.status(400).send({
          status: false,
          msg: "Title length should not be greater than 20",
        });
      if (!description)
        return res
          .status(400)
        ...
        return res
          .status(400)
          .send({ status: false, msg: "description length in between 10-80" });
      ...
      if (files1 && files1.length > 0) {
        ...
        return res
          .status(400)
          .send({ status: false, msg: "Please provide Product image" });
      }
      // ------------------------- validations end -----------------------------------
  ...
  getProducts: async function (req, res) {
    try {
      ...
      if (!title) {
        return res
          .status(400)
          .send({ status: false, msg: " Please use filter" });
      }
  ...
  updateProduct: async function (req, res) {
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(productId))
        return res
          .status(400)
          .send({ status: false, msg: "Invalid productId " });
      let data = req.body;
      if (data.length == 0)
        return res
          .status(400)
      ...
      if (!productDetails) {
        return res
          .status(404)
          .send({ status: false, msg: "no such product  exist" });
      }
      ...
      // ------------------------- validations end ------------------------------------
      let updatebook = await productModel.findOneAndUpdate(
      ...
      res.status(200).send({
        status: true,
        message: "Successfully Document Update",
        data: updatebook,
      });
    } catch (err) {
  ...
  deleteProduct: async function (req, res) {
    try {
      ...
      let product = await productModel.findOneAndUpdate(condition, {
