...
const createUser = async function (req, res) {
  try {
    ...
    if (unique) {
      return res
        .status(400)
        .send({ message: `${email}:--This maiId is already exist  ` }); //instead of 400 we can also use 409 for conflict
    }
...
const loginUser = async function (req, res) {
  try {
    ...
    // ------------------------- validations end -----------------------------------
    let token = jwt.sign({ userId: verifyUser._id.toString() }, "Sangram", {
      expiresIn: "24h",
    });
    res
      .status(200)
